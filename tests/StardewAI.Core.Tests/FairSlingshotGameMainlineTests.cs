...
public sealed class FairSlingshotGameMainlineTests
{
    [Fact]
    public void StrategyIsTrainableAndNativeTargetGameExecutorIsCalibrationOnly()
    {
        var strategy = OptionCapabilityRegistrySource.GetRequired("festival.play_slingshot_game");
        var executor = OptionCapabilityRegistrySource.GetRequired("executor.play_fair_slingshot_game");
        ...
        Assert.Equal(new[] { "EVD-294" }, strategy.RuntimeEvidenceIds);
        Assert.Equal(new[] { "EVD-294" }, strategy.OutputEvidenceIds);
        Assert.Contains("50_second_native_TargetGame", strategy.TrainingEvidenceScope, StringComparison.Ordinal);
        ...
        Assert.Equal(new[] { "EVD-294" }, executor.RuntimeEvidenceIds);
    }

    [Fact]
    public void FreshStardropDemandCompilesToOneNativeTargetGameSession()
    {
        var snapshot = Snapshot("fair-slingshot-a", remainingDemand: 1000);
        var availability = new CandidateOptionAvailabilityEvaluator()
            .Evaluate(snapshot, new[] { "festival.play_slingshot_game" }, true);
        var candidate = Assert.Single(Assert.Single(availability.Options).EventCandidates);

        Assert.True(candidate.Available, string.Join(";", candidate.BlockReasons));
        Assert.Equal("play_fair_slingshot_game", candidate.Kind);
        AssertParameter(candidate.Parameters, "entry_fee_money", "50");
        AssertParameter(candidate.Parameters, "game_duration_ms", "50000");
        AssertParameter(candidate.Parameters, "target_count", "79");
        AssertParameter(candidate.Parameters, "execution_strategy", "native_predictive_intercept_legal_input");

        var plan = new DailyPlanCompiler().Compile(
            new EventCandidateRanker().Rank(new BaselineTrainingReport(), availability), snapshot.StateHash);
        var step = Assert.Single(plan.Steps);
        Assert.Equal("play_fair_slingshot_game", step.Kind);
        Assert.Contains("one_native_50g_50_second_session_only", step.SafetyConstraints);
        Assert.Contains("no_direct_money_target_score_accuracy_reward_timer_or_inventory_mutation", step.SafetyConstraints);

        var queue = new ActionQueueCompiler().Compile(plan, snapshot);
        var item = Assert.Single(queue.Items);
        Assert.Equal("pending", queue.Status);
        Assert.Equal("executor.play_fair_slingshot_game", item.OptionId);
        Assert.Empty(item.BlockingReasons);
        Assert.Equal("play_fair_slingshot_game", Assert.Single(item.NormalizedCommand.Steps).StepType);
    }
    ...
    public void NoRemainingAutomaticDemandIsExcludedUpstream()
    {
        var snapshot = Snapshot(
            "fair-slingshot-complete",
            remainingDemand: 0,
            gateStatus: "complete_projected_tokens_cover_stardrop");
        var candidate = Assert.Single(Assert.Single(new CandidateOptionAvailabilityEvaluator()
            .Evaluate(snapshot, new[] { "festival.play_slingshot_game" }, true).Options).EventCandidates);

        Assert.False(candidate.Available);
        Assert.Contains("fair_slingshot_no_remaining_automatic_token_demand", candidate.BlockReasons);
    }

    [Fact]
    public void CompilerRejectsProjectionDriftBeforeRuntime()
    {
        var original = Snapshot("fair-slingshot-a", remainingDemand: 1000);
        var availability = new CandidateOptionAvailabilityEvaluator()
            .Evaluate(original, new[] { "festival.play_slingshot_game" }, true);
        var plan = new DailyPlanCompiler().Compile(
            new EventCandidateRanker().Rank(new BaselineTrainingReport(), availability), original.StateHash);
        var drifted = Snapshot("fair-slingshot-b", remainingDemand: 1000);
        plan.StateHash = drifted.StateHash;

        var queue = new ActionQueueCompiler().Compile(plan, drifted);

        Assert.Equal("blocked", queue.Status);
        Assert.Contains("fair_slingshot_game_projection_drifted", Assert.Single(queue.Items).BlockingReasons);
    }

    [Fact]
    public void RuntimeUsesNativeDialogueAndSharedSlingshotAimWithoutOutcomeWrites()
    {
        var root = FindRepositoryRoot();
        var runtime = File.ReadAllText(Path.Combine(
            root, "tools", "StardewAI.RuntimeTestHarness", "ModEntry.FairSlingshotGame.cs"));
        var slingshot = File.ReadAllText(Path.Combine(
            root, "tools", "StardewAI.RuntimeTestHarness", "ModEntry.Mining.Ranged.cs"));
        ...
        Assert.Contains("game.releaseLeftClick(0, 0)", runtime, StringComparison.Ordinal);
        Assert.Contains("SlingshotAimPatch", runtime, StringComparison.Ordinal);
        Assert.Contains("SlingshotAimPatch", slingshot, StringComparison.Ordinal);
        Assert.DoesNotContain("TargetGame.score = expected", runtime, StringComparison.Ordinal);
        Assert.DoesNotContain("TargetGame.score +=", runtime, StringComparison.Ordinal);
        Assert.DoesNotContain("TargetGame.accuracy = expected", runtime, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("RuntimeFairSlingshotGameEndTimerField?.SetValue", runtime, StringComparison.Ordinal);
    }

    private static SnapshotEnvelope Snapshot(string fingerprint, int remainingDemand, string gateStatus = "ready")
    {
        var targetSequence = string.Join(",", Enumerable.Range(0, 79).Select(index =>
            $$"""{"index":{{index}},"type":0,"spawn_time_ms":1000,"start_x":128,"start_y":128,"speed":2,"motion":0}"""));
        var json = $$$"""
        {
          "player": {
            ...
            "fair_slingshot_game":{"value":{
              "projection_status":"complete_current_festival_fall16_slingshot_game_context",
              ...
              "native_contract":"Event.checkAction(festival_fall16_buildings_501_502)->DialogueBox(slingshotGame:Play).receiveLeftClick->Event.answerDialogue(slingshotGame,0)->Money-50->globalFadeToBlack(TargetGame.startMe)->native_50000ms_TargetGame_input_session->accuracy_multiplier_score_reward->festivalScore",
              "interaction_tiles":[{"tile_x":10,"tile_y":9,"tile_index":501}],
