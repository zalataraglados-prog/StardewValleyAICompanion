...
public sealed class OptionRegistryGovernanceTests
{
    ...
    public void RegistryCountAndKindTests()
    {
        ...
        Assert.Equal(107, options.Count);
        Assert.Equal(37, options.Count(row => !row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        ...
        Assert.Equal(35, options.Count(row => row.SemanticKind == OptionSemanticKind.CompositeOptionSpec));
        Assert.Equal(70, options.Count(row => row.SemanticKind == OptionSemanticKind.PrimitiveOptionSpec));
    }
