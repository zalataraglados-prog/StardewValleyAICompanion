...
public sealed class OptionRegistryGovernanceTests
{
    ...
    public void RegistryCountAndKindTests()
    {
        ...
        Assert.Equal(120, options.Count);
        Assert.Equal(45, options.Count(row => !row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        ...
        Assert.Equal(43, options.Count(row => row.SemanticKind == OptionSemanticKind.CompositeOptionSpec));
