...
public sealed class OptionRegistryGovernanceTests
{
    ...
    public void RegistryCountAndKindTests()
    {
        ...
        Assert.Equal(229, options.Count);
        Assert.Equal(98, options.Count(row => !row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        ...
        Assert.Equal(85, options.Count(row => row.SemanticKind == OptionSemanticKind.CompositeOptionSpec));
