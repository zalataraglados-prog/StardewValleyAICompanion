...
public sealed class OptionRegistryGovernanceTests
{
    ...
    public void RegistryCountAndKindTests()
    {
        ...
        Assert.Equal(90, options.Count);
        Assert.Equal(31, options.Count(row => !row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        Assert.Equal(59, options.Count(row => row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        ...
        Assert.Equal(59, options.Count(row => row.SemanticKind == OptionSemanticKind.PrimitiveOptionSpec));
    }
