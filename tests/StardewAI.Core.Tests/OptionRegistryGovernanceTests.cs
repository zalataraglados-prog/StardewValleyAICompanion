...
public sealed class OptionRegistryGovernanceTests
{
    ...
    public void RegistryCountAndKindTests()
    {
        ...
        Assert.Equal(119, options.Count);
        Assert.Equal(44, options.Count(row => !row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        ...
        Assert.Equal(42, options.Count(row => row.SemanticKind == OptionSemanticKind.CompositeOptionSpec));
        Assert.Equal(75, options.Count(row => row.SemanticKind == OptionSemanticKind.PrimitiveOptionSpec));
    }
