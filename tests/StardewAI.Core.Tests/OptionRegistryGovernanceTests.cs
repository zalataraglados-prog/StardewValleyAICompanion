...
public sealed class OptionRegistryGovernanceTests
{
    ...
    public void RegistryCountAndKindTests()
    {
        ...
        Assert.Equal(116, options.Count);
        Assert.Equal(43, options.Count(row => !row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        ...
        Assert.Equal(41, options.Count(row => row.SemanticKind == OptionSemanticKind.CompositeOptionSpec));
