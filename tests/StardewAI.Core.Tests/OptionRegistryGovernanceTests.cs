...
public sealed class OptionRegistryGovernanceTests
{
    ...
    public void RegistryCountAndKindTests()
    {
        ...
        Assert.Equal(115, options.Count);
        Assert.Equal(42, options.Count(row => !row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        ...
        Assert.Equal(40, options.Count(row => row.SemanticKind == OptionSemanticKind.CompositeOptionSpec));
