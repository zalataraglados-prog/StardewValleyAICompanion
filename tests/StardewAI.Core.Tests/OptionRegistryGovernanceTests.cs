...
public sealed class OptionRegistryGovernanceTests
{
    ...
    public void RegistryCountAndKindTests()
    {
        ...
        Assert.Equal(123, options.Count);
        Assert.Equal(47, options.Count(row => !row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        ...
        Assert.Equal(45, options.Count(row => row.SemanticKind == OptionSemanticKind.CompositeOptionSpec));
