...
public sealed class OptionRegistryGovernanceTests
{
    ...
    public void RegistryCountAndKindTests()
    {
        ...
        Assert.Equal(89, options.Count);
        Assert.Equal(31, options.Count(row => !row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        Assert.Equal(58, options.Count(row => row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        ...
        Assert.Equal(58, options.Count(row => row.SemanticKind == OptionSemanticKind.PrimitiveOptionSpec));
    }
