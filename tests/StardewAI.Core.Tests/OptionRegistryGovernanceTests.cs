...
public sealed class OptionRegistryGovernanceTests
{
    ...
    public void RegistryCountAndKindTests()
    {
        ...
        Assert.Equal(122, options.Count);
        Assert.Equal(46, options.Count(row => !row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        ...
        Assert.Equal(44, options.Count(row => row.SemanticKind == OptionSemanticKind.CompositeOptionSpec));
        Assert.Equal(76, options.Count(row => row.SemanticKind == OptionSemanticKind.PrimitiveOptionSpec));
    }
