...
public sealed class OptionRegistryGovernanceTests
{
    ...
    public void RegistryCountAndKindTests()
    {
        ...
        Assert.Equal(92, options.Count);
        Assert.Equal(31, options.Count(row => !row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        Assert.Equal(61, options.Count(row => row.OptionId.StartsWith("executor.", StringComparison.Ordinal)));
        ...
        Assert.Equal(61, options.Count(row => row.SemanticKind == OptionSemanticKind.PrimitiveOptionSpec));
    }
