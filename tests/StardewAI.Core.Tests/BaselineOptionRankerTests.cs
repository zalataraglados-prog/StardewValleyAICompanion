...
namespace StardewAI.Core.Tests
{
    public sealed class BaselineOptionRankerTests
    {
        ...
        public void RankUsesOnlyEvidenceAdmittedOptionsWhenCandidatesAreEmpty()
        {
            ...
            Assert.Equal(62, prediction.RankedOptions.Length);
            ...
            Assert.Contains(prediction.RankedOptions, item => item.OptionId == "story.advance_event" && item.Evidence == "unseen_option");
            Assert.Contains(prediction.RankedOptions, item => item.OptionId == "tailoring.sew_item" && item.Evidence == "unseen_option");
            ...
            Assert.Contains(prediction.RankedOptions, item => item.OptionId == "minigame.play_calico_jack" && item.Evidence == "unseen_option");
            Assert.Contains(prediction.RankedOptions, item => item.OptionId == "minigame.play_darts" && item.Evidence == "unseen_option");
            Assert.Contains(prediction.RankedOptions, item => item.OptionId == "minigame.play_prairie_king" && item.Evidence == "unseen_option");
            Assert.Contains(prediction.RankedOptions, item => item.OptionId == "minigame.play_slots" && item.Evidence == "unseen_option");
            Assert.Contains(prediction.RankedOptions, item => item.OptionId == "mining.activate_calico_statue" && item.Evidence == "unseen_option");
            ...
            Assert.Contains(prediction.RankedOptions, item => item.OptionId == "rewards.claim_adventure_guild_reward" && item.Evidence == "unseen_option");
            Assert.Contains(prediction.RankedOptions, item => item.OptionId == "rewards.claim_prize_ticket" && item.Evidence == "unseen_option");
