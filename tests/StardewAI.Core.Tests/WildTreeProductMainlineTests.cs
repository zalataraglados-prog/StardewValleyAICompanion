...
public sealed class WildTreeProductMainlineTests
{
    ...
    [Fact]
    public void FallHazelnutCarriesExactSeedDropRowInsteadOfBaseSeedSource()
    {
        var snapshot = Snapshot(StateJson(
            ...
            4,
            treeType: "2",
            branch: "fall_hazelnut_replaces_seed",
            outputQualifiedItemId: "(O)408",
            routeKind: "native_wild_tree_seed_drop",
            sourceId: "wild_tree:2:0"));
        var candidate = Assert.Single(
            new CandidateOptionAvailabilityEvaluator()
                .Evaluate(
                    snapshot,
                    new[] { "foraging.harvest_tree_product" },
                    true)
                .Options.Single().EventCandidates);

        Assert.True(candidate.Available,
            string.Join(";", candidate.BlockReasons));
        Assert.Equal("(O)408", candidate.QualifiedItemId);
        var sources = candidate.Parameters.Single(parameter =>
            parameter.Name == "authoritative_route_sources_json").Value;
        Assert.Contains("native_wild_tree_seed_drop", sources,
            StringComparison.Ordinal);
        Assert.Contains("wild_tree:2:0", sources,
            StringComparison.Ordinal);
        Assert.DoesNotContain("native_wild_tree_seed\"", sources,
            StringComparison.Ordinal);
    }
    ...
    private static string StateJson(
        ...
        int safeSlot,
        string treeType = "1",
        string branch = "default_seed",
        string outputQualifiedItemId = "(O)309",
        string routeKind = "native_wild_tree_seed",
        string sourceId = "wild_tree:1") => $$$"""
    {
      "player": {
      ...
      "current_location":{"debris":{"value":[],"status":"available"},"terrain_features":{"value":[{
        "tile_x":12,"tile_y":10,"type":"StardewValley.TerrainFeatures.Tree","runtime_type":"StardewValley.TerrainFeatures.Tree","tree_type":"{{{treeType}}}",
        ...
        "tree_product_harvest_status":"{{{status}}}","tree_product_data_contract_status":"exact_locked_base_1.6.15","tree_product_branch":"{{{branch}}}",
        "tree_product_guaranteed_outputs":[{"qualified_item_id":"{{{outputQualifiedItemId}}}","quality":0,"quantity":1,"branch":"{{{branch}}}"}],
        ...
        "tree_product_authoritative_route_sources":[{"route_kind":"{{{routeKind}}}","source_id":"{{{sourceId}}}","qualified_item_id":"{{{outputQualifiedItemId}}}"}],
