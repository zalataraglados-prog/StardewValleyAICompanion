param(
    ...
    [string] $SaveSlot = "",
    [string] $RunId = ("runtime-flooring-placement-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
    [int] $TargetTileX = 64,
    [int] $TargetTileY = 15,
...
function Wait-World([string] $Url, [int] $Seconds) {
    ...
    while ((Get-Date) -lt $deadline) {
        try {
            ...
            if ($snapshot.state.player.location_id.status -in @("available", "derived") -and
                $snapshot.state.player.flooring_placement.status -in @("available", "derived") -and
    ...
    throw "Timed out waiting for flooring-placement snapshot. Last error: $last"
}
function New-Request($Snapshot, [string] $OptionId, [string] $QueueItemId) {
    [ordered]@{
        schema_version = "training_execution_request.v1"
        run_id = $RunId
        queue_id = "runtime-flooring-placement"
...
foreach ($port in @(8765, 8767)) {
    if ($null -ne (Get-NetTCPConnection -State Listen -LocalPort $port -ErrorAction SilentlyContinue)) {
        throw "Flooring-placement smoke requires unused port $port."
    }
}
...
$artifactDirectory = Join-Path $ProjectRoot ("artifacts\runtime-flooring-placement\" + $RunId)
...
try {
    ...
    $catalog = @($initial.state.player.flooring_placement.value.floor_path_catalog | Where-Object {
        [bool]$_.is_item_lookup_target -and -not [string]::IsNullOrWhiteSpace([string]$_.qualified_item_id)
    })
    if ($catalog.Count -eq 0) { throw "Live Data/FloorsAndPaths exposed no canonical inventory flooring rows." }
    $cases = @()
    foreach ($catalogRow in $catalog) {
        $qualifiedItemId = [string]$catalogRow.qualified_item_id
        $fixtureRequest = New-Request $initial "debug.setup_flooring_placement_target" "$RunId.fixture.$($catalogRow.floor_data_key)"
        ...
        if ($fixture.status -ne "applied" -or $fixture.primitive_verification_status -ne "verified") {
            throw "Flooring fixture failed for ${qualifiedItemId}: $(@($fixture.block_reasons) -join ',')"
        }
        ...
        $placement = $before.state.player.flooring_placement.value
        $row = @($placement.rows | Where-Object {
            [string]$_.qualified_item_id -eq $qualifiedItemId -and
            [string]$_.floor_data_key -eq [string]$catalogRow.floor_data_key -and [int]$_.stack -gt 0
        }) | Select-Object -First 1
        if ($null -eq $row) { throw "Transparent flooring inventory row is missing for $qualifiedItemId" }
        ...
        if ($null -eq $range) { throw "Transparent flooring target range is missing for $qualifiedItemId" }

        $applyRequest = New-Request $before "executor.place_flooring" "$RunId.place.$($catalogRow.floor_data_key)"
        ...
        $applyRequest["floor_data_key"] = [string]$row.floor_data_key
        $applyRequest["flooring_connect_type"] = [string]$row.connect_type
        $applyRequest["expected_flooring_neighbor_mask"] = [int]$range.expected_neighbor_mask_after
        $applyRequest["expected_flooring_view_min"] = [int]$row.expected_which_view_min
        $applyRequest["expected_flooring_view_max"] = [int]$row.expected_which_view_max
        ...
        $placed = @($after.state.current_location.terrain_features.value | Where-Object {
            ...
            [string]$_.type -eq "StardewValley.TerrainFeatures.Flooring"
        }) | Select-Object -First 1
        $state = $placed.flooring_state
        $transparentVerified = $null -ne $placed -and [string]$state.status -eq "available" -and
            [string]$state.floor_data_key -eq [string]$row.floor_data_key -and [bool]$state.is_passable -and
            [int]$state.derived_neighbor_mask -eq [int]$range.expected_neighbor_mask_after -and
            [int]$state.which_view -ge [int]$row.expected_which_view_min -and
            [int]$state.which_view -le [int]$row.expected_which_view_max
        $passed = $result.status -eq "applied" -and
            $result.primitive_verification_status -eq "verified" -and $transparentVerified
        $cases += [ordered]@{
            qualified_item_id = $qualifiedItemId
            floor_data_key = [string]$row.floor_data_key
            connect_type = [string]$row.connect_type
            target_tile = "$TargetTileX,$TargetTileY"
            expected_neighbor_mask = [int]$range.expected_neighbor_mask_after
            observed_which_view = if ($null -eq $placed) { $null } else { [int]$state.which_view }
        ...
        if (-not $passed) { throw "Runtime flooring placement failed for $qualifiedItemId" }
        $initial = $after
    }

    $summary = [ordered]@{
        schema_version = "stardewai.runtime_flooring_placement_smoke.v1"
        status = if (@($cases | Where-Object { $_.status -ne "passed" }).Count -eq 0) { "passed" } else { "failed" }
        run_id = $RunId
        save_slot = $SaveSlot
        live_catalog_count = $catalog.Count
