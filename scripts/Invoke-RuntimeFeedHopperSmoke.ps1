...
param(
    ...
    [string] $SaveSlot = "",
    [string] $RunId = ("runtime-feed-hopper-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
    [string] $OutputDirectory = "artifacts\runtime-feed-hopper",
...
try {
    ...
    $fixture = Invoke-JsonPost $executeUrl (New-Request $current "debug.setup_feed_hopper" "$RunId.fixture")
    Write-Json (Join-Path $runDirectory "fixture.json") $fixture
    if ($fixture.status -ne "applied") { throw "Feed Hopper fixture failed: $(@($fixture.block_reasons) -join ',')" }
    Start-Sleep -Milliseconds 500

    $ready = Wait-Snapshot $snapshotUrl 30
    $hopper = @($ready.state.current_location.objects.value | Where-Object { $null -ne $_.feed_hopper_withdrawal }) | Select-Object -First 1
    if ($null -eq $hopper) { throw "Feed Hopper transparent projection is missing." }
    $projection = $hopper.feed_hopper_withdrawal
    ...
    if ($projection.status -ne "ready" -or $null -eq $stand) { throw "Feed Hopper projection is not executable: $($projection.status)" }
    if ($safe.safe_slot_kind -notin @("empty", "tool")) { throw "No safe toolbar slot is available." }

    $request = New-Request $ready "animals.withdraw_feed_hopper_hay" "$RunId.withdraw"
    $request.location_id = [string]$ready.state.player.location_id.value
    $request.target_tile_x = [int]$hopper.tile_x
    $request.target_tile_y = [int]$hopper.tile_y
    ...
    $request.feed_hopper_safe_slot_kind = [string]$safe.safe_slot_kind
    $request.restore_slot_index = [int]$safe.current_tool_index
    $request.feed_hopper_hay_qualified_item_id = [string]$projection.hay_qualified_item_id
    $request.feed_hopper_root_location_id = [string]$projection.root_location_id
    $request.feed_hopper_silo_hay_before = [int]$projection.silo_hay_before
    $request.feed_hopper_animal_count = [int]$projection.animal_count
    $request.feed_hopper_animal_limit = [int]$projection.animal_limit
    $request.feed_hopper_placed_hay_count = [int]$projection.placed_hay_count
    $request.feed_hopper_unfed_animal_count = [int]$projection.unfed_animal_count
    $request.feed_hopper_expected_withdrawal_quantity = [int]$projection.expected_withdrawal_quantity
    $request.feed_hopper_expected_silo_hay_after = [int]$projection.expected_silo_hay_after
    $request.feed_hopper_expected_location_action_return = [bool]$projection.expected_native_location_action_return
    ...
    $afterHopper = @($after.state.current_location.objects.value | Where-Object {
        [int]$_.tile_x -eq [int]$hopper.tile_x -and [int]$_.tile_y -eq [int]$hopper.tile_y
    }) | Select-Object -First 1
    $expectedQuantity = [int]$projection.expected_withdrawal_quantity
    $passed = $result.status -eq "applied" -and
        ...
        $result.observed_effect -match "native_handled=true" -and
        $result.observed_effect -match "silo_hay=$([int]$projection.expected_silo_hay_after)" -and
        $null -ne $afterHopper -and
        [string]$afterHopper.qualified_item_id -eq "(BC)99" -and
        [int]$after.state.player.current_tool_index.value -eq [int]$safe.current_tool_index
    $summary = [ordered]@{
        schema_version = "stardewai.runtime_feed_hopper_smoke.v1"
        run_id = $RunId
        status = if ($passed) { "passed" } else { "failed" }
        execution_status = $result.status
        verification = $result.primitive_verification_status
        expected_withdrawal_quantity = $expectedQuantity
