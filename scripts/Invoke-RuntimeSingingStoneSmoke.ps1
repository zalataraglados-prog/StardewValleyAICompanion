...
param(
    ...
    [string] $SaveSlot = "",
    [string] $RunId = ("runtime-singing-stone-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
    [string] $OutputDirectory = "artifacts\runtime-singing-stone",
...
function New-Request($Snapshot, [string] $OptionId, [string] $ItemId) {
    [ordered]@{
        ...
        run_id = $RunId
        queue_id = "runtime-singing-stone"
...
try {
    ...
    $fixture = Invoke-JsonPost $executeUrl (New-Request $current "debug.setup_singing_stone" "$RunId.fixture")
    Write-Json (Join-Path $runDirectory "fixture.json") $fixture
    if ($fixture.status -ne "applied") { throw "Singing Stone fixture failed: $(@($fixture.block_reasons) -join ',')" }
    Start-Sleep -Milliseconds 500

    $ready = Wait-Snapshot $snapshotUrl 30
    $stone = @($ready.state.current_location.objects.value | Where-Object { $null -ne $_.singing_stone_interaction }) | Select-Object -First 1
    if ($null -eq $stone) { throw "Singing Stone transparent projection is missing." }
    $projection = $stone.singing_stone_interaction
    ...
    if ($projection.status -ne "ready" -or $null -eq $stand) { throw "Singing Stone projection is not executable." }
    if ($safe.safe_slot_kind -notin @("empty", "tool")) { throw "No safe toolbar slot is available." }
    if ($projection.exact_next_pitch_status -ne "unavailable_shared_rng_state_not_consumed") {
        throw "Transparent projection must not expose or consume the shared RNG's next pitch."
    }

    $request = New-Request $ready "world.play_singing_stone" "$RunId.play"
    $request.location_id = [string]$ready.state.player.location_id.value
    $request.target_tile_x = [int]$stone.tile_x
    $request.target_tile_y = [int]$stone.tile_y
    ...
    $request.singing_stone_safe_slot_kind = [string]$safe.safe_slot_kind
    $request.restore_slot_index = [int]$safe.current_tool_index
    $request.singing_stone_sound_name = [string]$projection.sound_name
    $request.singing_stone_pitch_rng_source = [string]$projection.pitch_rng_source
    $request.singing_stone_exact_next_pitch_status = [string]$projection.exact_next_pitch_status
    $request.singing_stone_pitch_min = [int]$projection.pitch_min_inclusive
    $request.singing_stone_pitch_max = [int]$projection.pitch_max_inclusive
    $request.singing_stone_pitch_step = [int]$projection.pitch_step
    $request.singing_stone_pitch_outcome_count = [int]$projection.pitch_outcome_count
    $request.singing_stone_expected_shake_timer = [int]$projection.expected_shake_timer_immediately_after_action
    $request.singing_stone_expected_location_action_return = [bool]$projection.expected_native_location_action_return
    ...
    $afterStone = @($after.state.current_location.objects.value | Where-Object {
        [int]$_.tile_x -eq [int]$stone.tile_x -and [int]$_.tile_y -eq [int]$stone.tile_y
    ...
    $passed = $result.status -eq "applied" -and
        ...
        $result.observed_effect -match "shake_timer=100" -and
        $null -ne $afterStone -and
        [string]$afterStone.item_id -eq "94" -and
        [string]$afterStone.qualified_item_id -eq "(BC)94" -and
        [int]$after.state.player.current_tool_index.value -eq [int]$safe.current_tool_index
    $summary = [ordered]@{
        schema_version = "stardewai.runtime_singing_stone_smoke.v1"
        run_id = $RunId
        ...
        exact_next_pitch_status = $projection.exact_next_pitch_status
        pitch_distribution = $projection.pitch_distribution
