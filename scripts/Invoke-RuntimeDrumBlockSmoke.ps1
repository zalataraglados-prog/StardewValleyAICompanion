...
param(
    ...
    [string] $SaveSlot = "",
    [string] $RunId = ("runtime-drum-block-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
...
$runDirectory = Join-Path $ProjectRoot (Join-Path "artifacts\runtime-drum-block" $RunId)
...
function Request($Snapshot, [string] $Option, [string] $Item) {
    [ordered]@{ schema_version = "training_execution_request.v1"; run_id = $RunId; queue_id = "runtime-drum-block"; queue_item_id = $Item;
...
try {
    ...
    $fixture = Post-Json $executeUrl (Request $current "debug.setup_drum_block" "$RunId.fixture")
    if ($fixture.status -ne "applied") { throw "Drum Block fixture failed: $(@($fixture.block_reasons) -join ',')" }
    Start-Sleep -Milliseconds 500
    $ready = Wait-Snapshot $snapshotUrl 30
    $row = @($ready.state.current_location.objects.value | Where-Object { $null -ne $_.drum_block_tuning }) | Select-Object -First 1
    if ($null -eq $row) { throw "Drum Block projection missing." }
    $p = $row.drum_block_tuning
    $stand = @($p.stand_tiles | Where-Object { $_.available }) | Select-Object -First 1
    $safe = $ready.state.player.safe_item_context.value
    $request = Request $ready "world.tune_drum_block" "$RunId.tune"
    ...
    $request.safe_slot_index = [int]$safe.safe_slot_index; $request.drum_block_safe_slot_kind = [string]$safe.safe_slot_kind; $request.restore_slot_index = [int]$safe.current_tool_index
    $request.drum_block_current_tone_raw = [string]$p.current_tone_raw; $request.drum_block_current_tone = [int]$p.current_tone_parsed; $request.drum_block_next_tone = [int]$p.next_tone
    $request.drum_block_tone_min = [int]$p.tone_min_inclusive; $request.drum_block_tone_max = [int]$p.tone_max_inclusive; $request.drum_block_tone_step = [int]$p.tone_step
    $request.drum_block_tone_state_count = [int]$p.tone_state_count; $request.drum_block_sound_cue = [string]$p.sound_cue
    $request.drum_block_expected_shake_timer = [int]$p.expected_shake_timer_immediately_after_action; $request.drum_block_expected_scale_y = [double]$p.expected_scale_y_immediately_after_action
    $request.drum_block_expected_location_action_return = [bool]$p.expected_native_location_action_return; $request.interaction_kind = [string]$p.interaction_kind
    ...
    $passed = $result.status -eq "applied" -and $result.primitive_verification_status -eq "verified" -and $result.training_impact_scope -eq "player_command_only_executor_evidence" -and
        [string]$afterRow.drum_block_tuning.current_tone_raw -eq [string]$p.next_tone -and [string]$afterRow.qualified_item_id -eq "(O)463" -and
        [int]$after.state.player.current_tool_index.value -eq [int]$safe.current_tool_index
    $summary = [ordered]@{ schema_version = "stardewai.runtime_drum_block_smoke.v1"; run_id = $RunId; status = if ($passed) { "passed" } else { "failed" };
        ...
        tone_before = [int]$p.current_tone_parsed; tone_after = [int]$afterRow.drum_block_tuning.current_tone_parsed; sound_cue = [string]$p.sound_cue;
        observed_effect = $result.observed_effect; loaded_mod_allowlist = $mods; output_directory = $runDirectory }
