param(
    ...
    [string] $SaveSlot = "",
    [string] $RunId = ("runtime-fair-fishing-game-" + (Get-Date -Format "yyyyMMdd-HHmmss")),
    ...
    while ((Get-Date) -lt $deadline) {
        try {
            ...
            if ($snapshot.state.player.location_id.status -in @("available", "derived") -and
                $snapshot.state.player.fair_fishing_game.status -in @("available", "derived")) { return $snapshot }
        } catch { }
        Start-Sleep -Seconds 2
    }
    throw "Timed out waiting for fair fishing game snapshot."
}
function Wait-FairFishing([string] $Url, [int] $Seconds) {
    $deadline = (Get-Date).AddSeconds($Seconds)
    while ((Get-Date) -lt $deadline) {
        $snapshot = Wait-World $Url 15
        $context = $snapshot.state.player.fair_fishing_game.value
        if ($context.festival_id -eq "festival_fall16" -and $context.gate_status -eq "ready") {
            return $snapshot
        }
        Start-Sleep -Seconds 1
    }
    throw "Timed out waiting for a ready fall16 fair fishing game context."
}
function New-BaseRequest($Snapshot, [string] $OptionId, [string] $QueueItemId) {
    [ordered]@{
        schema_version = "training_execution_request.v1"; run_id = $RunId; queue_id = "runtime-fair-fishing-game"
...
function Select-AdjacentInteraction($Snapshot) {
    ...
    $row = @($Snapshot.state.player.fair_fishing_game.value.interaction_tiles | Where-Object {
        ([Math]::Abs([int]$_.tile_x - $px) + [Math]::Abs([int]$_.tile_y - $py)) -eq 1
    }) | Select-Object -First 1
    if ($null -eq $row) { throw "Fixture player is not adjacent to a projected fair fishing interaction tile." }
    return $row
}
function New-FairFishingRequest($Snapshot, [string] $QueueItemId) {
    $context = $Snapshot.state.player.fair_fishing_game.value
    $interaction = Select-AdjacentInteraction $Snapshot
    $request = New-BaseRequest $Snapshot "executor.play_fair_fishing_game" $QueueItemId
    $request.location_id = [string]$context.festival_location_id
    $request.max_movement_tiles = 512
    $request.fair_fishing_projection_fingerprint = [string]$context.projection_fingerprint
    $request.fair_fishing_interaction_tile_x = [int]$interaction.tile_x
    $request.fair_fishing_interaction_tile_y = [int]$interaction.tile_y
    $request.fair_fishing_stand_tile_x = [int]$Snapshot.state.player.tile_x.value
    $request.fair_fishing_stand_tile_y = [int]$Snapshot.state.player.tile_y.value
    $request.fair_fishing_money_before = [int]$context.player_money
    $request.fair_fishing_entry_fee_money = [int]$context.entry_fee_money
    $request.fair_fishing_festival_score_before = [int]$context.festival_score
    $request.fair_fishing_stardrop_price_star_tokens = [int]$context.stardrop_price_star_tokens
    $request.fair_fishing_projected_unclaimed_grange_tokens = [int]$context.projected_unclaimed_grange_tokens
    $request.fair_fishing_remaining_star_token_demand = [int]$context.remaining_star_token_demand
    $request.fair_fishing_game_duration_ms = [int]$context.game_duration_ms
    $request.fair_fishing_results_duration_ms = [int]$context.results_duration_ms
    $request.fair_fishing_dialogue_key = [string]$context.dialogue_key
    $request.fair_fishing_play_response_key = [string]$context.play_response_key
    $request.fair_fishing_execution_strategy = [string]$context.execution_strategy
...
$artifactDirectory = Join-Path $ProjectRoot ("artifacts\runtime-fair-fishing-game\" + $RunId)
...
try {
    ...
    $fixtureRequest = New-BaseRequest $snapshot "debug.setup_fair_fishing_game" "$RunId.setup"
    ...
    if ($fixture.status -ne "applied" -or $fixture.primitive_verification_status -ne "verified") {
        throw "Fair fishing fixture failed: $($fixture.observed_effect)"
    }
    $snapshot = Wait-FairFishing $snapshotUrl 60
    $result = Invoke-Post $executeUrl (New-FairFishingRequest $snapshot "$RunId.play") 300
    if ($result.status -ne "applied" -or $result.primitive_verification_status -ne "verified") {
        throw "Fair fishing game failed: status=$($result.status); reasons=$($result.block_reasons -join ','); observed=$($result.observed_effect)"
    }
    ...
    $summary = [ordered]@{
        schema_version = "stardewai.runtime_fair_fishing_game_smoke.v1"
        status = "passed"
        run_id = $RunId
