...
- **Authoritative acquisition calendar sources** - The four Stage 1 collection denominators retain all 1,599
  ...
  shares the native condition normalizer with Data/Locations, and binds all 75 `Data/Crops` harvest routes to
  exact seed rows. Crop evidence includes native season windows, `SeedsIgnoreSeasonsHere` alternates, phase and
  regrow timing, watering/paddy/location-rule dependencies and complete wild-seed output domains. The static
  report resolves 580 sources and explicitly blocks 1,019 unsupported route kinds; every resolved route remains
  target-date pending and cannot authorize training. Shop stock/opening conditions are the next source family.
