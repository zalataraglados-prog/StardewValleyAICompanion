...
- **Authoritative acquisition calendar sources** - The four Stage 1 collection denominators retain all 1,599
  ...
  remains false. The following `opportunity_cost` axis is implemented on its dedicated branch and awaits the deferred
  regression run. It preserves guaranteed elapsed minutes, native energy, exact material quantities partitioned by
  ...
  cost identity fails closed. After regression and merge, the only remaining fixed axis is a fresh terminal receipt
  boundary; training authorization remains false until that boundary and route-composition admission are complete.
