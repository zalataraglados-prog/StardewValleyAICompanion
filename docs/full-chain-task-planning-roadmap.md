...
- **Vanilla Incubator lifecycle** - The exact `Object.OutputIncubator` model resolves animal type, native duration and unscaled native purchase value, while reserving one future AnimalHouse slot for each active incubator. Core admits loading only when the current snapshot has an unreserved slot and carries the exact prediction through daily planning and queue validation. At maturity, the bridge exposes the native first-ready incubator order and `NamingMenu`; runtime clicks the native done button and verifies exact animal creation, occupancy growth, egg removal and menu closure. EVD-156 closes the hidden isolated chicken-hatch naming path. Natural multi-day incubation and the remaining egg/building runtime matrix remain pending.
