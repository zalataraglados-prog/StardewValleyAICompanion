...
- **Generic machine placement** - Inventory machines expose lossless native legal ranges for every loaded persistent location. The loaded-map head chooses an exact reachable tile and adjacent stand; compiler and dispatch rebind inventory identity, legal range, projection fingerprint, operational context, and material-ledger freshness. The runtime uses only `Utility.playerCanPlaceItemHere` and `Utility.tryToPlaceItem`. Hidden isolated smokes verify current-map placement and the full `Farm -> FarmHouse` remote chain: exact candidate selection, native building-door traversal, fresh snapshot, exact target-map tile, matching machine object, and one-item consumption. EVD-150 now composes this placement head into ordinary strategic relocation across one resolved connector. Multi-connector runtime coverage remains pending.
- **Empty-fleet machine lifecycle and strategic relocation** - A repeatable hidden isolated Furnace run starts with zero loaded Furnaces, uses the production native menu to craft one, places that exact inventory item, loads Copper Ore with native additional Coal consumption, waits the native 30 game minutes, and collects the predicted Copper Bar. It then verifies transparent idle/removal safety, calls the native Pickaxe removal chain, observes exact machine recovery through debris followed by adjacent auto-collection, and reuses native placement at a different legal tile. The transparent placement projection probes every distinct recoverable placed-machine type against native-legal current and relevant target-map ranges without scanning unrelated empty maps. Core retains one positive-benefit relocation candidate per target location over the explicit eight-cycle policy horizon. Backend persists source/target/fingerprint plus connector count, live route estimate, relocation cost, benefit policy and target-selection policy as one save/player-scoped intent before dispatch. After removal and recovery, generic source-map placement is suppressed; a remote intent may emit only its target connector, then requires a fresh target-map snapshot to rebind exact legality, stand, inventory identity and native placement. Until a remote path projection is added, the committed remote target must be native-legal and directly adjacent to the resolved connector arrival, which is the proven stand. Hidden isolated proofs close same-map Keg relocation `56,15 -> 49,15` and one-connector relocation `Farm 56,15 -> FarmHouse 27,29`, including native remove/recovery/placement, transparent door traversal for the latter, distinct state hashes and completed intents. Deeper target-map layouts and multi-connector relocation remain fail closed. Workbench lifecycle evidence and special/conditional/random or multiplayer matrices remain pending.
