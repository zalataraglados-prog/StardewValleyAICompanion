...
- **Empty-fleet machine lifecycle and mechanical relocation** - A repeatable hidden isolated Furnace run starts with zero loaded Furnaces, uses the production native menu to craft one, places that exact inventory item, loads Copper Ore with native additional Coal consumption, waits the native 30 game minutes, and collects the predicted Copper Bar. It then verifies transparent idle/removal safety, calls the native Pickaxe removal chain, observes exact machine recovery through debris followed by adjacent auto-collection, and reuses native placement at a different legal tile. `executor.remove_machine` requires an explicit relocation intent and is not emitted as an autonomous daily candidate. Cross-location composition, Workbench lifecycle evidence, special/conditional/random machine families, and the strategic layout policy that decides when relocation is worthwhile remain pending.
