...
- **Authoritative acquisition calendar sources** - The four Stage 1 collection denominators retain all 1,599
  ...
  first post-route settlement/checkpoint and verified continuation Teacher denominator are also implemented. Formal
  training authorization remains false until the selected continuation is committed and dispatched under fresh
  ownership, multi-transition rollout reaches a verified portfolio completion, and controller admission consumes that
  proof.
...
carry the selected continuation through fresh commit/dispatch/settlement ownership, repeat the
checkpoint chain, order the remaining routes and emit one portfolio-level completion receipt before
formal Teacher rollout admission. Completed alternative identity already enters the fresh Teacher
denominator and cannot be asserted by an ordinary caller proposal.
