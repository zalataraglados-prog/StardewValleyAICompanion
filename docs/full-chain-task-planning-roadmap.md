...
- **Generic storage placement** - Inventory player-storage objects have a separate `storage_placement.v1` projection covering ordinary/stone, Big, Junimo, Mini-Fridge, Mini-Shipping, and live inventory-`Chest` branches. The current-map head selects one native-legal tile only when virtual placement preserves the walkable component, map endpoints, and every existing storage access stand. Candidate, plan, compiler, dispatch, and runtime preserve exact item/branch/type/capacity/role, route-safe layout, topology fingerprint, and material-ledger identity. Runtime uses only `Utility.playerCanPlaceItemHere` and `Utility.tryToPlaceItem`; an isolated ordinary chest smoke verified the new access point and exact one-item consumption. Remote persistent locations execute one connector with an explicit storage continuation and defer exact tile selection until a fresh target-map snapshot. Special-branch smokes, full cross-location runtime, acquisition, purpose/demand, explicit sharing, and relocation remain pending.
