...
- **Authoritative acquisition calendar sources** - The four Stage 1 collection denominators retain all 1,599
  ...
  exact same-snapshot route to the fishing stand or an adjacent authoritative shop-action stand, and fits the complete
  ...
  rolling compiler's maximum menu/interact/dialogue/purchase/close duration for the complete purchase count. The
  ...
  second-route fresh receipt/settlement, a cumulative two-transition completion checkpoint and a linear repeatable
  proof-manifest verifier are also implemented. The rollout controller now recomputes the manifest, rejects a
  caller-authored receipt, blocks incomplete chains, and admits only the exact terminal proof as scoped Teacher
  evidence. This scoped admission does not authorize `formal_product_training`; a three-or-more-transition fixture and
  portfolio supervision dataset adapter remain before the repeated path can feed formal training.
...
contains two transitions; a three-or-more-transition fixture remains an explicit regression gate.
