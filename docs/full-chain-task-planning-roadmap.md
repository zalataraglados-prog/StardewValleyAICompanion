...
- **Authoritative acquisition calendar sources** - The four Stage 1 collection denominators retain all 1,599
  ...
  second-route fresh receipt/settlement and a cumulative two-transition completion checkpoint are also implemented.
  Formal training authorization remains false until this proof becomes an arbitrary-depth transition chain and
  controller admission consumes its verified terminal checkpoint.
...
generalize the verified two-transition path into a repeatable checkpoint chain, order every remaining
route and emit one terminal portfolio-level completion proof before formal Teacher rollout admission.
