...
- **`raise_skill_levels` transparent input** — `player.skills_detail` now exposes all six vanilla skill rows, including unmodified/effective levels, temporary buff deltas, exact accumulated experience, next-level thresholds, remaining experience, the level cap, and the Grandpa scoring formula. The direction remains deliberately blocked until existing mechanical candidates carry decompile-backed experience yields; transparent inputs alone are not treated as an executable skill-training policy.
