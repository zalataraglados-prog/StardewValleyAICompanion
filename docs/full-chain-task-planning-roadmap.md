...
- **Mining native floor-step follow-up** - `MiningFloorStepPlanner` now decodes the transparent compact collision grid and uses deterministic BFS to select a reachable ladder, required monster, or ladder-producing stone. Internal `executor.mine_stone` walks and swings a real pickaxe through the native tool lifecycle; isolated E: verified natural stone removal plus truthful swing/health feedback. Combat, ladder/shaft interaction, descent confirmation, retreat, and the repeated multi-floor loop remain pending, so `mining.reach_depth` stays blocked.
...
- Fishing candidates compile to collision-safe stand movement plus `executor.catch_fish`. Mining reach-depth remains intentionally blocked until combat, ladder/shaft interaction, descent confirmation, retreat, and repeated after-snapshot replanning are complete. The verified internal one-stone primitive is executor calibration infrastructure, not a new small-model action.
