...
- **Mining rolling floor-step execution** - `mining.reach_depth` remains the model-facing objective and compiles exactly one current-floor primitive before the next transparent snapshot. The compiler can select native stone mining, native auto-combat, natural debris pickup, native food recovery, native ladder/shaft descent, or mandatory native mine exit with deterministic BFS and dynamic threat interruption. Time, energy, and unrecoverable-health boundaries become `executor.exit_mine` upstream instead of stranding the actor. Loaded vanilla MineShaft stones now expose the decompile-derived complete set of possible item identities, separated into guaranteed, guaranteed-one-of, and conditional outputs; a high-level resource target can select its source without model-supplied stone IDs and prefers guaranteed nodes. Monster projections now distinguish raw spawn-selected drops from the effective MineShaft death branch, correctly suppress base drops for special-item monsters, preview the current death-tile special item without claiming it survives movement, and expose honest guaranteed/conditional unions. All eleven vanilla `getExtraDropItems` overrides are projected from live type-specific fields, including guaranteed held items, guaranteed-one-of dinosaur drops, slime color/special state, and the whole-branch pendant override. Shared cosmetic catalogs, hard-mine treasure completion, exact source probabilities, full-objective time calibration, and isolated multi-floor runtime validation remain pending.
