...
- **Grandpa marriage/house factor, farmhouse axis** - `world_progress.marriage_house` now keeps marriage/roommate, engagement, spouse, pending-roommate, current farmhouse level, construction countdown, Robin/counter availability, other-construction state, balance, and the exact next vanilla upgrade tuple together. `housing.advance_farmhouse` emits one `purchase_farmhouse_upgrade` candidate only when the native Carpenter branch is currently legal. The compiler and runtime independently rebind level, countdown, money, materials, endpoint, stand tile, Robin, and the three-day construction result. Runtime uses only `GameLocation.checkAction` plus native `carpenter_Upgrade` and `upgrade_Yes` responses; it does not write money, inventory, house level, or countdown. The Grandpa direction can bind this house-axis candidate, but after house level 2 it still fails closed until the separate proposal/roommate completion chain is implemented. Static focused regression passes; irreversible isolated in-game purchase smoke remains pending.
