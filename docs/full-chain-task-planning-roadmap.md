...
- **Authoritative acquisition calendar sources** - The four Stage 1 collection denominators retain all 1,599
  ...
  is separate from trainer readiness: the current one-candidate-per-transition fixture has no pairwise preference
  and remains trainer-blocked. This scoped artifact does not authorize `formal_product_training`; multi-candidate
  strict-Pareto coverage across all splits and a dedicated goal-to-method trainer remain before the repeated path can
  feed formal training. The existing action ranker may not reinterpret `candidate.Selected` as these labels.
