...
- **Ordinary story-event execution** - `player.story_event` exposes the active vanilla `Event` as a lossless command/response projection with explicit festival, minigame and player-control ownership boundaries. `story.advance_event` emits either one automatic continuation or one candidate per exact native dialogue response; the compiler rebinds event identity, location, command index/raw text, question/response and projection fingerprint. The runtime advances only through native `Event.Update` and menu input, stops at completion or the next fresh owner boundary, and never calls `skipEvent` or writes event command/seen/dialogue state. EVD-322 closes hidden automatic-message and second-response fixtures. Event-owned minigames remain the next separate action and must reuse their existing native minigame state machines.
