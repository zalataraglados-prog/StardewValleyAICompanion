...
- **Mining rolling floor-step execution** - `mining.reach_depth` remains the model-facing objective and compiles exactly one current-floor primitive before the next transparent snapshot. The compiler can select native stone mining, native auto-combat, natural debris pickup, native food recovery, native ladder descent, or native Skull Cavern shaft descent with deterministic BFS and dynamic threat interruption. Shaft depth and damage are previewed from the exact local deterministic formula and verified after landing. Full-objective time calibration, retreat/exit policy, exhaustive resource-source mapping, and isolated multi-floor runtime validation remain pending.
