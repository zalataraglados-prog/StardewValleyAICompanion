...
- **Authoritative acquisition calendar sources** - The four Stage 1 collection denominators retain all 1,599
  ...
  first post-route settlement/checkpoint, verified continuation Teacher denominator, continuation commit receipt and
  second-route dispatch ownership are also implemented. Formal training authorization remains false until that route is
  executed and settled, multi-transition rollout reaches a verified cumulative portfolio completion, and controller
  admission consumes that proof.
...
- **Persistent material and currency commitments** - The save/player strategy ledger stores exact actor-authorized node/slot/item reservations and locked native-currency reservations with goal ownership, optimistic revisions, cancellation, and immutable history. The native currency domain is exactly money, star tokens, club coins and Qi gems, shared from one contract by the bridge, target-date budget evaluator and controller ledger; malformed balances, wrong ownership and cross-route overbooking fail closed. Machine-crafting candidates exclude any exact personal or Workbench consumption plan that would spend reserved stock; the daily plan and action compiler bind and recompute ledger ID, revision, guard status, and relevant reservation IDs. Immediately before runtime input, the training controller queries backend dispatch readiness against the latest ledger; stale queues send no game input and are recompiled with bounded retries. Exact per-route claims, scoped portfolio admission, bounded strict-Pareto Teacher preference, one-revision atomic whole-set commit, post-commit exact-active-set receipt, queue ownership binding, exact completed-route settlement, a first-transition rollout checkpoint, verified completed-alternative carry-forward into a fresh remaining-scope Teacher denominator, continuation commit receipt and second-route dispatch ownership are implemented. The checkpoint can prove exact one-route portfolio completion; incomplete work cannot reuse stale IDs. Continuation execution/settlement, a second cumulative checkpoint, evidence-backed incomparable-portfolio policy and final controller admission remain before formal training authorization.
...
execute and settle the selected continuation under its fresh ownership, repeat the checkpoint chain,
order the remaining routes and emit one portfolio-level completion receipt before formal Teacher
rollout admission. Completed alternative identity already enters the fresh Teacher denominator and
the continuation commit/dispatch proof; it cannot be asserted by an ordinary caller proposal.
