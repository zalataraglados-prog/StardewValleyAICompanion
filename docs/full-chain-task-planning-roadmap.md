...
- **Mining rolling floor-step execution** - `mining.reach_depth` remains the model-facing objective and compiles exactly one current-floor primitive before the next transparent snapshot. The compiler can select native stone mining, native auto-combat, natural debris pickup, native food recovery, or native ladder descent with deterministic BFS and dynamic threat interruption. Static regression is complete; shaft jumping, full-objective time calibration, retreat/exit policy, exhaustive resource-source mapping, and isolated multi-floor runtime validation remain pending.
