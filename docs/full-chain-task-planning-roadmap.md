...
- **Authoritative acquisition calendar sources** - The four Stage 1 collection denominators retain all 1,599
  ...
  `student_observation` channels. This scoped artifact does not authorize `formal_product_training`; multi-rollout
  dataset governance and a trainer that consumes Teacher labels instead of `candidate.Selected` remain before the
  repeated path can feed formal training.
