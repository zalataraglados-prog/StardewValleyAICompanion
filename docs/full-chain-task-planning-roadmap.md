...
- **Empty-fleet machine lifecycle** - A repeatable hidden isolated Furnace run now starts with zero loaded Furnaces, uses the production native menu to craft one, places that exact inventory item, loads Copper Ore with the native additional Coal consumption, waits the native 30 game minutes, and collects the predicted Copper Bar from the same object. All four production primitives verify their post-state and the machine returns idle. The bridge now matches the decompiled `ItemContextTagManager` effective capability rule instead of treating raw `MachineData.HasInput/HasOutput` force flags as final; this closes a field omission that previously hid Furnace and other rule-inferred input machines. Cross-location composition, Workbench lifecycle evidence, special/conditional/random machine families, strategic layout maintenance, removal, and relocation remain pending.
