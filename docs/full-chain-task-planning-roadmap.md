...
- **Persistent material commitments** - The save/player strategy ledger now stores exact actor-authorized node/slot/item reservations with goal ownership, optimistic revisions, cancellation, and immutable history. Machine-crafting candidates exclude any exact personal or Workbench consumption plan that would spend reserved stock; the daily plan and action compiler bind and recompute ledger ID, revision, guard status, and relevant reservation IDs. A latest-ledger check immediately before runtime dispatch remains required before concurrent planner execution is enabled.
