...
- **Generic machine placement** - Inventory machines expose lossless native legal ranges for every loaded persistent location. The loaded-map head chooses an exact reachable tile and adjacent stand; compiler and dispatch rebind inventory identity, legal range, projection fingerprint, operational context, and material-ledger freshness. The runtime uses only `Utility.playerCanPlaceItemHere` and `Utility.tryToPlaceItem`, and hidden isolated smoke verifies the machine object plus exact one-item consumption. A remote location head now presents each transparently legal reachable destination, executes one connector while preserving slot/item/location identity, refreshes the snapshot, and then invokes the loaded-map exact-tile head. Cross-location runtime smoke remains pending.
- **Empty-fleet machine lifecycle and strategic same-map relocation** - A repeatable hidden isolated Furnace run starts with zero loaded Furnaces, uses the production native menu to craft one, places that exact inventory item, loads Copper Ore with native additional Coal consumption, waits the native 30 game minutes, and collects the predicted Copper Bar. It then verifies transparent idle/removal safety, calls the native Pickaxe removal chain, observes exact machine recovery through debris followed by adjacent auto-collection, and reuses native placement at a different legal tile. The transparent placement projection also probes every distinct recoverable placed-machine type against every native-legal tile on the current loaded map; the current-map scope is explicit so it does not multiply native probes across irrelevant persistent locations. Core emits at most one current-map relocation candidate when an idle source has a native-legal reachable target whose nearest-machine service route produces positive net savings over the explicit eight-cycle policy horizon. Backend persists the accepted source/target/fingerprint evidence as one save/player-scoped relocation intent before dispatch; the removal compiler and dispatch-readiness gate require that active intent. After the mandatory fresh snapshot, the source-present state re-emits the same safe removal head, while the source-missing state suppresses new layout decisions until recovery. Once the matching machine is in inventory, `place_machine_item` selects only the persisted exact target, rechecks its current native legal range and stand geometry, and the placement compiler rebinds the active intent; observing the exact target completes the ledger entry. Hidden isolated E-drive proof now closes this autonomous chain for a positive-benefit Keg relocation `56,15 -> 49,15`: native remove/recovery, fresh-snapshot exact placement, three distinct state hashes, and completed intent all passed. The shared movement primitive settles the farmer at the stand-tile center before immediate native placement, eliminating transient farmer-collision rejection without teleportation. Cross-location composition, Workbench lifecycle evidence, and special/conditional/random or multiplayer matrices remain pending.
