...
- 未完成或多路线组合不会沿用旧 route ID，而是输出 `fresh_replan_required=true`。首检查点本身不授权下一路线；下述 continuation 边界已经闭合“累计完成 alternative -> 最新 state/ledger 剩余 Teacher 分母 -> 提交回执 -> 下一路线派发绑定”，但下一路线的执行、结算与累计完成仍未闭合，因此 `formal_training_authorized=false`。
...
- continuation Teacher 选出的 proposal/admission 现可经 `build-acquisition-route-portfolio-continuation-commit-receipt` 和 `build-acquisition-route-continuation-execution-binding` 进入既有提交与派发证明。回执和绑定都携带 prior checkpoint 哈希及累计完成 alternatives，并要求 proposal/admission/receipt 三者一致；没有新 claim 的续轮继续走已存在 reservation 的零变更精确证明。
- 双路线离线正例已通过：商店路线先完成并结算，首检查点留下钓鱼路线；fresh state/settled ledger 随后只枚举未完成钓鱼 scope，生成续接提交回执并绑定第二路线队列。Release 构建为 0 warnings / 0 errors，聚焦四收集/76 路线自测通过。
- 正式训练继续关闭。下一固定切片是让第二路线沿 continuation proof 执行并结算，再将前后两次转换合并为累计 rollout checkpoint；在第二次转换能被整链精确重放前，不得宣称通用多轮组合完成。
