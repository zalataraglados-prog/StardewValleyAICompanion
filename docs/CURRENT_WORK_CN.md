...
- 未完成或多路线组合不会沿用旧 route ID，而是输出 `fresh_replan_required=true`。首检查点本身不授权下一路线；下述 continuation 边界只闭合“累计完成 alternative -> 最新 state/ledger 剩余 Teacher 分母”，后续提交、执行、结算与累计完成仍未闭合，因此 `formal_training_authorized=false`。
...
- 正式训练继续关闭。下一固定切片是把 continuation Teacher 选出的 proposal/admission 接入原子 commit receipt、route execution binding、下一次 settlement 和累计 rollout checkpoint；在第二次转换能被整链精确重放前，不得宣称通用多轮组合完成。
