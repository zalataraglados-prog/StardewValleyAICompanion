...
- 该准入只允许这条 portfolio 作为 Teacher 证据进入后续数据适配，不启动或授权 `formal_product_training`，也不绕过 option allowlist、数据集/checkpoint、Product Executor、版本锁和原生存档边界。全局正式训练继续关闭。下一固定切片是把已准入 portfolio 适配为显式监督来源的数据行。
- 验证通过：GoalConditionedBootstrap Release `0 warning / 0 error`，完整 Stage 1 collection self-test、独立 proof/admission CLI、Core game-free `81/81`、Backend `203/203`。本切片未启动游戏，也未写正式训练数据。
