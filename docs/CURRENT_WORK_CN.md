...
- 未完成或多路线组合不会沿用旧 route ID，而是输出 `fresh_replan_required=true`。首检查点本身不授权下一路线；下述 continuation 边界已经闭合“累计完成 alternative -> 最新 state/ledger 剩余 Teacher 分母 -> 提交回执 -> 下一路线派发 -> fresh 终态 -> 结算 -> 第二检查点”，但任意深度重复链与正式 controller 准入仍未闭合，因此 `formal_training_authorized=false`。
...
- continuation Teacher 选出的 proposal/admission 现可经对应 continuation CLI 进入既有提交、派发、fresh 回执、结算和累计检查点证明。所有产物都从首检查点 proof 重建，回执和绑定携带 prior checkpoint 哈希及累计完成 alternatives，并要求 proposal/admission/receipt 三者一致。
...
- 正式训练继续关闭。下一固定切片是把当前“首检查点 + 第一次 continuation”的专用 proof 推广为可重复 N 次的统一 transition proof/checkpoint 链，再让 rollout controller 只接纳完整终态证明；在此之前不得把双路线夹具称为通用任意深度 rollout。
