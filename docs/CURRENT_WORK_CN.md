...
- `acquisition_route_portfolio_rollout_proof_receipt.v1` 已绑定 manifest 哈希、latest checkpoint 哈希、transition count、终态 state/ledger 和完成状态。双路线正例、篡改 checkpoint 拒绝和独立 CLI 重放通过；代码路径不限制 continuation 数量，但当前真实正例仍只有两次 transition。
...
- 该准入只允许这条 portfolio 作为 Teacher 证据进入后续数据适配，不启动或授权 `formal_product_training`，也不绕过 option allowlist、数据集/checkpoint、Product Executor、版本锁和原生存档边界。全局正式训练继续关闭。下一固定切片是补一个三次以上 transition 的真实链式夹具，再把已准入 portfolio 适配为显式监督来源的数据行。
