...
- 未完成或多路线组合不会沿用旧 route ID，而是输出 `fresh_replan_required=true`。首检查点本身不授权下一路线；下述 continuation 边界已经闭合“累计完成 alternative -> 最新 state/ledger 剩余 Teacher 分母 -> 提交回执 -> 下一路线派发 -> fresh 终态 -> 结算 -> 下一检查点”。线性 proof manifest 可按同一实现重复任意轮次，但正式 controller 准入仍未闭合，因此 `formal_training_authorized=false`。
...
- 正式训练继续关闭。下一固定切片是让 rollout controller 只接纳 `proof_chain_verified=true` 且 `portfolio_completion_verified=true` 的终态 proof receipt，并补一个三次以上 transition 的实测夹具；在 controller 准入完成前不得宣称正式 rollout 已获授权。
