...
- 验证通过：GoalConditionedBootstrap Release `0 warning / 0 error`，完整 Stage 1 collection self-test；正例精确生成 3 行并保持 3 Teacher / 3 native / 0 Student observation，全部负例通过。本切片未启动游戏，也未写正式训练数据。下一固定切片是建立该 schema 的多 rollout 聚合、分割和 trainer 准入，并移除 `StructuredPolicyTrainer.BuildPairs` 对 `candidate.Selected` 的正例依赖；19/19 Teacher 覆盖仍是正式全量训练的独立硬门。
