...
源哈希漂移、候选 ID 缺失和非准入选择均失败关闭。清洗、正式切分、数据集哈希清单和长回报
回填仍未完成，因此本节退出条件尚未满足。
...
当前代码任务处于第 2 至第 4 步之间：注册表框架已存在，普通矿井与火山已有部分真实状态闭环，但最近可见测试暴露了共享执行底座的可信度缺口。下一步先完成第 3 步，不能用更多目标族代码、服务器无渲染运行或增加训练轮次掩盖底层输入问题。底座通过后，从权威字典差异矩阵恢复第 4 步，不进行一次性“大而全”改写。
