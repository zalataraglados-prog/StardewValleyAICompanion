...
该语义当前是“编译完成、训练未准入”。既有 EVD-153 和 EVD-214 分别证明底层制作/摆放/处理链与
有约束首次投料，但尚缺由本高层选项驱动的跨快照隐藏隔离整链证据。因此 allowlist 仍为 17，不能
提前把组合证据登记为 EVD-215。下一退出条件是原生制作、摆放、首次投料、处理开始、意图完成和训练
记录在同一隔离运行中全部成立；随后才更新五门登记并进入任务/收集需求机器切片。
