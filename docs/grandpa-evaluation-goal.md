...
Machine realization is location-bound rather than inferred from the capacity projection. The compatibility field `farm.machines` now enumerates machine objects through the live persistent location/interior topology, so Sheds, Cabins, Greenhouse/farm roots, animal-building interiors, assigned FarmHouse/Cellar, and player-owned machines placed on other persistent maps are not omitted. Every row carries its exact `location_id`, ownership/root/parent-building evidence, and no machine-row truncation. `player.machine_placement` separately calls native placement rules for every inventory machine and emits lossless legal-tile ranges per persistent location. The current-map chain selects and rebinds one exact reachable tile, moves adjacent, checks the latest material ledger, and executes native placement with an exact post-state receipt. The remote output head exposes transparently legal reachable locations to the small model, routes one connector while retaining slot/item/location identity, refreshes the target map, and only then selects the exact tile from current collision evidence. The small model, not the executor, must compare capacity, path, round-trip time and maintenance frequency. This does not by itself justify buying level 3: the utility model must compare process demand, machine throughput, input availability, route cost, existing spare capacity, and the 100,000g opportunity cost.
