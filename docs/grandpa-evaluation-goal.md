...
Machine realization is location-bound rather than inferred from the capacity projection. The compatibility field `farm.machines` enumerates all machine objects in the player's Farm, assigned FarmHouse, and assigned Cellar, with an explicit live `location_id` on every row and no machine-row truncation. Same-map collection/loading uses only that map's occupancy and collision grid. Remote work emits one resolved route connector and requires a fresh snapshot before selecting a stand tile or probing an input. This closes the read/route/compile/runtime location chain, but it does not by itself justify buying level 3: the remaining utility model must compare process demand, machine throughput, input availability, route cost, existing spare capacity, and the 100,000g opportunity cost.
