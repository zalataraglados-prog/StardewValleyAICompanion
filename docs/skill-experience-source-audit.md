...
| `Crop.hitWithHoe` / `HoeDirt.performToolAction` ginger branch | Foraging | exact current ginger identity, Hoe slot, energy, `(O)829 x1` debris, soil after-state, and `+7` XP flow through a dedicated candidate/compiler/native-tool chain. Hidden/silent isolated matrix passed dry standard-tool collection, rainy Efficient full-inventory world-debris retention, and insufficient-energy upstream exclusion on 2026-07-30. |
| `Bush.performUseAction` / `Bush.shake` | Foraging for ordinary berries; none for tea/walnut branches | every exact current `largeTerrainFeatures` Bush exposes footprint, size/age/shelter/readiness/bloom/cooldown, branch, exact output, Botanist quality, `1 + ForagingLevel / 4` berry quantity and matching XP, and golden-walnut tracker key; candidate/compiler bind a reachable perimeter action tile; runtime uses native `checkAction` and verifies offset, combined debris/inventory output, XP, and nut tracker without direct mutation. Hidden/silent isolated 2026-07-30 matrix passed ordinary quality-zero berries, Botanist iridium berries, tea, mutex-backed golden walnut, already-collected exclusion, and shake-cooldown exclusion. |
...
| mine treasure `Chest` collection | Luck call site, effective XP zero | every loaded ordinary-Mine/Skull-Cavern reward chest exposes exact chest/reward identity and native lifecycle; candidate/compiler/runtime chain is implemented. Decompiled `gainExperience(5, 25 + mineLevel)` is ignored by `Farmer.gainExperience` for skill 5, so the exact observed Luck XP delta is zero rather than a trainable skill gain. Hidden/silent isolated matrix `runtime-mine-reward-chest-smoke-20260730-004220` passed an ordinary/remixed reward, the floor-100 Stardrop, and a forced two-chest Skull Cavern layout. Quarry Mine and Volcano rewards remain separate action families. |
