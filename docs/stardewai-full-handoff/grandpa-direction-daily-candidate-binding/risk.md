...
**Risk**: 5 of 12 directions are unconditionally blocked as planned contract gaps. No speculative field/capability checking is performed -- the catalog merely records what would be needed. Newly added `raise_skill_levels` source families and `earn_pet_love` runtime paths remain runtime-pending and fail closed on incomplete evidence.
