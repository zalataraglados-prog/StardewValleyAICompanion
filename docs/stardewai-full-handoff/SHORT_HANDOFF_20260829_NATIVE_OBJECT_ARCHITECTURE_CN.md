...
`animals.collect_auto_grabber_contents` 完成 issue #88 的增量架构迁移；随后
`movement.use_mini_obelisk` 按同一边界接入。
...
- 六类活动状态归入 `NativeObjectInteractionDomainState`，根 `ModEntry` 只负责一次 tick/reset/is-active 委派。
...
Mini-Obelisk 只复用该底层；其原生容器顺序、成对身份、远端选择、落点顺序和延迟传送回执是第六套独立契约。
...
- Mini-Obelisk：隐藏 E 盘通过，`runtime-mini-obelisk-20260829-011139`，预期/实际落点 `(1,2)`，原生配对身份与工具栏槽恢复。
