...
> 2026-09-08 当前覆盖说明：本文保留当时的工程事实，但其中“已观察选择作为 pairwise 正例”的行为克隆语义不得用于新的正式全量训练。当前合同见 `../TEACHER_STUDENT_CONVERGENCE_CONTRACT_CN.md`。新 schema 必须分离 Teacher preference、native outcome 与 Student observation；`selected=true` 本身不是正标签。
...
当前 V1 是透明的历史行为克隆基线，返回值只调整已观察选择的训练权重，不提供反事实最优性证明，且不再满足当前正式训练准入。
