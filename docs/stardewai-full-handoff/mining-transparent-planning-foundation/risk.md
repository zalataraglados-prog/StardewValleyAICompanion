...
- Volcano transparency and planning use the separate `volcano` domain and `volcano.reach_caldera` option. MineShaft-only combat/tool/descent state is not reused. Volcano cooling, obstacle, and supported-melee primitives have independent native-input state machines; unsupported target semantics, stale identities, dynamic path drift, and missing tools remain explicit blockers.
