...
- Volcano transparency and planning use the separate `volcano` domain and `volcano.reach_caldera` option. MineShaft-only combat/tool/descent executors are not reused; missing cooling, combat, or obstacle primitives remain explicit compiler blockers.
