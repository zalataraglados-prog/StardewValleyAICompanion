...
`acquisition_route_portfolio_continuation_teacher_request.v1` now carries an incomplete first checkpoint into the next current-state Teacher denominator. The builder first reproduces that checkpoint, then requires the exact terminal state and settled-ledger hash/revision. Completed alternatives are removed internally and remaining slots are recalculated before the same bounded enumeration, strict-Pareto selection and atomic admission preflight run. Caller-authored completion fields are rejected. The selected continuation reuses the exact commit-receipt and execution-binding implementations while preserving the checkpoint hash and completed-alternative set across proposal, admission and receipt. It still needs execution, settlement and a second cumulative checkpoint before another transition is complete.
...
This closes controller persistence for the listed commitment and reservation types, atomic material/currency portfolio mutation, post-commit ownership proof, strict-Pareto Teacher selection, per-route dispatch binding, exact completed-route settlement, the first rollout checkpoint, verified remaining-scope continuation selection, continuation commit receipt and second-route dispatch binding, not the entire long-horizon planner. Greenhouse/Island/Indoor Pot rules, fertilizer and skill modifiers, crop layout feasibility, evidence-backed resolution of incomparable route portfolios, continuation execution/settlement, multi-transition portfolio completion, future-income commitments, animal/building commitments, mining/smelting queues, storage supply, and broader machine placement/service still require separate typed commitments or transparent state.
