...
`acquisition_route_portfolio_rollout_proof_manifest.v1` is the repeatable proof surface. It contains one initial proof and an ordered continuation-transition list. Verification rebuilds every checkpoint, prior hash, transition count, state and ledger identity before exposing the latest checkpoint; `acquisition_route_portfolio_rollout_proof_receipt.v1` binds the manifest and latest checkpoint hashes. A two-transition positive fixture and tamper rejection pass. `acquisition_route_portfolio_rollout_admission_receipt.v1` is the controller boundary: it recomputes the manifest, requires the supplied proof receipt to equal that result, blocks incomplete portfolios, and admits only a terminal chain as scoped Teacher evidence. It does not authorize `formal_product_training`. The implementation has no fixed continuation count, while a three-or-more-transition fixture and portfolio dataset adapter remain explicit gates.
