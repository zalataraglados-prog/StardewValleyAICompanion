...
`acquisition_route_portfolio_rollout_proof_manifest.v1` is the repeatable proof surface. It contains one initial proof and an ordered continuation-transition list. Verification rebuilds every checkpoint, prior hash, transition count, state and ledger identity before exposing the latest checkpoint; `acquisition_route_portfolio_rollout_proof_receipt.v1` binds the manifest and latest checkpoint hashes. A three-transition shop/ready-crop/fishing fixture and tamper rejection pass, including fresh forecast evidence after each state change. `acquisition_route_portfolio_rollout_admission_receipt.v1` is the controller boundary: it recomputes the manifest, requires the supplied proof receipt to equal that result, blocks incomplete portfolios, and admits only a terminal chain as scoped Teacher evidence. `acquisition_route_portfolio_supervision_dataset.v1` then rebuilds both receipts and emits one hash-linked row per transition with typed, non-interchangeable Teacher, native-outcome and Student-observation channels. The Teacher rollout records Student observation as unavailable instead of inventing a label. `acquisition_route_portfolio_supervision_corpus_manifest.v1` re-verifies multiple sources, deduplicates exact rows, rejects conflicting identities, and partitions only by verified save-day context. It independently blocks trainer admission when pairwise Teacher signal or split coverage is absent. Three independent save-day sources now provide real strict-Pareto pairs in every partition, and the dedicated `explicit_teacher_pairwise_portfolio_ranker.v1` re-verifies those sources before training only from explicit Teacher pairs. Fresh-state scoring now rebuilds the complete admitted denominator: unique strict-Pareto selection stays Teacher-owned, while an incomparable frontier can receive only a read-only checkpoint-ranked shadow choice. `goal_method_teacher_coverage_gate.v1` independently rebuilds the 19-criterion frontier and derives coverage from verified source evidence; its current count is 0/19 ready (2 comparison-covered, 4 native-outcome-covered). None of these artifacts authorize `formal_product_training`.
...
This closes controller persistence for the listed commitment and reservation types, atomic material/currency or marker-only portfolio mutation, post-commit ownership proof, strict-Pareto Teacher selection, per-route dispatch binding, exact completed-route settlement, verified continuation selection/execution/settlement, cumulative three-transition completion, the repeatable proof-chain verifier and scoped terminal rollout admission, not the entire long-horizon planner. Greenhouse/Island/Indoor Pot rules, fertilizer and skill modifiers, crop layout feasibility, evidence-backed resolution of incomparable route portfolios, portfolio dataset adaptation, future-income commitments, animal/building commitments, mining/smelting queues, storage supply, and broader machine placement/service still require separate typed commitments or transparent state.
