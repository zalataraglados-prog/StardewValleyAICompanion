...
爷爷事件接续仍未在实际出现该事件的服务器克隆上复验，正式训练保持关闭。
...
准入边界不得扩大解释：本轮 `formal_training_started=false`，且该克隆没有出现爷爷事件，
`storyEventAdvanceCount=0`。因此已闭合的是“两项教师社交 + 一次原生跨日”，爷爷自动剧情
接续仍须在确实触发该事件的隔离存档上单独通过。当前服务器仅 2 核、3.6 GiB 内存，保留
...
回执覆盖完整后，确定性搜索老师可以独立生成正确存档。模型用于压缩长程搜索成本、提高
自然度并适应玩家偏好；在老师尚未覆盖完整目标图时，模型不能替老师填补未知规则。
