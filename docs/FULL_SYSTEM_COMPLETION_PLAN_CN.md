...
目标严格分两阶段：阶段一从新档开始，在第三年春 1 日首次爷爷评估取得原版全部 `21/21`
分；阶段二以阶段一策略检查点为热启动，把目标条件扩展为原版 Perfection 追踪器 `100%`。
第二阶段可以让模型提出新路线并通过老师重标来自动扩大覆盖，但不能靠模型猜测新增规则；
Perfection 新增的依赖、透明字段、候选与原生回执仍须进入同一权威图和五门验收。

本地实验先证明了两个精确社交目标和一个原生跨日审计，119 测试服务器随后复现了同一
有界范围；`r44` 又用同一老师闭环实测通过内容寻址压缩快照的写入、读回和跨日使用。
`r46` 随后在第 3 年春 1 日隔离克隆上完成爷爷事件 `558291` 的唯一原生执行链。
正式训练仍保持关闭。
服务器只允许并发 1、无渲染、克隆存档、有界准入，并须逐日哈希回传。迁移到服务器是为了
不干扰交互电脑，不代表可以跳过版本、磁盘、性能、来源存档和剧情决策门。

## 2026-09-06 服务器准入实证与后续分工

`r43` 已在 119 服务器完成单并发、隐藏、静音、克隆存档、`--skip-training` 的有界运行。
确定性教师先后选择 Robin 和 Abigail；既有候选、唯一 DailyPlan/动作编译器及原生执行器
分别完成 23 与 17 个 verified 原语，友谊 `362 -> 382`、`154 -> 174`，随后由既有恢复链
关闭对话框。原生睡眠把春 18 日推进到春 19 日，38 个 NPC、29 条友谊记录的结算审计为
`0 mismatch`。来源存档前后树哈希均为
`ad9a78a1b652d1557041d9d7f15b8f711ce082ecbe1c1894953cd2bbfad3dede`；完整 519 文件、
372,400,193 字节证据已回传 I 盘，服务器与本地聚合 SHA-256 同为
`28cb2d1fef494c17bbb5eb91b26570a0c5a11450ec11280331076b595cdac46d`。

本轮也修正了两个只能由真实长路由暴露的问题：编译器现在按命令携带的明确坐标核验并行
连接器，不再误验图中的第一条边；反编译确认 Backwoods 的 `asdlfkjg` 是可穿过的偶发秘密
触发器而非路障，其限定时段、天气、单人和 2.5% 随机副作用仍完整记录。回归为 Core
`2423/2423`、Backend `177/177`、KnowledgeCompiler `585/585` 且 blocking 0，能力矩阵
`228/151/62`。

`r46` 使用归档的第 3 年春 1 日克隆，运行前确认 `558291` 尚未进入 `eventsSeen`。第一次派发
因自动剧情在编译后从命令 6 前进到命令 7 而按严格指纹阻塞；fresh 快照重编译后，既有
`story.advance_event -> executor.advance_story_event` 链以 19 次原生对话点击完成事件，产生
恰好一条 verified 回执，事件后状态为 inactive。来源存档树哈希前后均为
`221d9c7d5f0cd3aa4406b6b81950b4e576b8048070249f1900c51f0a2e974ac5`，完整 161 文件证据
已回传并逐文件校验。`formal_training_started=false`，所以它闭合的是剧情执行准入，不是
21 分老师或模型训练完成。当前服务器仅 2 核、3.6 GiB 内存，保留
证据后根盘余量约 5.4 GB；它适合承担 Slice 3 至 Slice 6 的单并发游戏闭环，不适合 GPU
训练或无界留存。`r44` 将 96 个快照的 315,482,766 逻辑字节压缩为 10,181,914 字节的
清单与内容块，节省约 96.77%；全部 2400 次块引用、1013 个唯一块和重建后的逻辑哈希均
通过独立校验。源码修改、构建和回归留在本机；清单锁定的发布包、克隆存档运行与证据生成
放服务器；Slice 7 正式模型训练仍使用 RTX 5070 节点。

最终 100% 存档老师不以学生模型为必需条件：当目标依赖图、规则、候选、动作编译与原生
回执覆盖完整后，确定性搜索老师可以独立生成正确存档。实际开发采用更经济的课程：先用
完整 21 分老师训练目标条件模型，再把 Perfection 目标与新依赖逐批开放，由同一老师验证
和重标模型到访状态。模型用于压缩长程搜索成本、提高自然度并适应玩家偏好；在老师尚未
覆盖完整目标图时，模型不能替老师填补未知规则。
