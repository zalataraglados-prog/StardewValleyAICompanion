...
训练证据注册表和由证据生成的 allowlist 已于 2026-07-28 完成首版：
...
当前继续回填已有运行证据并生成准确缺口清单；随后立即进入真实能力缺口的动作编译器、执行器和结果回写开发。之后才是重建策略轨迹、接入 C# 结构化排序器和长期完整 rollout。短训只能做基础设施烟测，不得替代正式全量训练。
