...
## 2026-08-05 任务驱动机器容量组合（运行时证据待完成）
...
无关预留只有在既有材料图证明精确输入槽余量充足时才可通过。Core 1528/1528、Backend 111/111 已通过。

这还不是 EVD-217，也不改变训练 allowlist。下一退出条件是在 E 盘隐藏静默隔离存档完成普通任务与特别订单矩阵：
无容量制作路径、背包机器直接摆放路径、规划器绑定后的原生摆放、任务进度不变的精确原生投料、自然处理、原生收取后
任务进度增加、意图完成，以及任务漂移和预留冲突负例。制品和特征行通过后，才能更新能力注册表、权威对账、证据表和
训练 allowlist。
