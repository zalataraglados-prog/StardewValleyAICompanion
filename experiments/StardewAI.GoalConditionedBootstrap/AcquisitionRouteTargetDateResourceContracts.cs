...
public sealed class AcquisitionRouteTargetDateResourceReport
{
    ...
    public string AdmissionPolicy { get; set; } =
        "The resource_inputs axis runs only after an exact facility-capacity match. It reads canonical actor-authorized immediately available material nodes, not a player-inventory-only approximation. Existing matching crops consume no new seed; otherwise a crop needs one exact seed item. Shop barter items are resources while money and other native currencies remain owned by currency_budget. A fishing route needs Magic Bait only when every retained target-date window requires it. Existing crab-pot output, loaded bait or owner Luremaster satisfies current input service; an unserviced pot fails closed until its exact native-accepted bait candidate domain is bound. Reusable tools, facility construction, processing time, stochastic retries, reservations, daily budget and terminal receipts remain independently owned. Unknown or deferred route kinds cannot default to no input, and training authorization stays false.";
}
