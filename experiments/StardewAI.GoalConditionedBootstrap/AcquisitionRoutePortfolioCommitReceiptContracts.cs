...
public sealed class AcquisitionRoutePortfolioCommitReceipt
{
    ...
    public string AdmissionPolicy { get; set; } =
        "The receipt deterministically rebuilds the pre-commit route portfolio admission and verifies the exact committed ledger. A mutating admission must match one accepted commit result, advance exactly one revision, preserve every requested claim as an exact active row, cancel every explicit release and record all component history plus one portfolio marker at the committed revision. An already-committed admission must observe an unchanged ledger and the same exact active claim set. This proves reservation ownership only; route execution and fresh terminal outcomes remain separate, so formal training authorization stays false.";
}
