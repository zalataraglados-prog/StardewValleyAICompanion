...
public sealed partial class ActionQueueCompiler
{
        ...
        CompileCraftStorageItemStep(SmallModelAction action)
    {
        ...
        return new[]
        {
            Step(
                ...
                outputQualifiedId + ":" + outputCount.Value +
                ";player.crafting_recipes[" + recipeName +
                "].count_increases=" + outputCount.Value,
                30)
        };
    }

    private static string[] ValidateCraftStorageItemPlan(
        SmallModelAction action,
        SnapshotEnvelope snapshot,
        StrategyCommitmentLedger? commitmentLedger)
    {
        if (action.OptionId != "executor.craft_storage_item")
        {
        ...
        {
            reasons.Add(
                "craft_storage_item_menu_must_be_clear");
        }

        var recipeName = ReadParameter(action, "recipe_name");
        var row = StorageCraftingRow(snapshot, recipeName);
        if (!row.HasValue)
        {
            reasons.Add(
                "craft_storage_item_recipe_not_verified_by_transparent_state");
            return reasons.ToArray();
        }

        var craftingSource = ReadParameter(
            action,
            "crafting_source");
        ...
        var source = usesWorkbench
            ? StorageWorkbenchCraftingSource(
                row.Value,
                ReadParameter(
                    action,
        ...
        {
            reasons.Add(
                "craft_storage_item_source_not_verified_by_transparent_state");
            return reasons.ToArray();
        }

        var expectedIngredients =
            source.Value.TryGetProperty(
                "ingredient_rows",
                out var ingredients)
        ...
        if (!string.Equals(
            ...
            true)
        {
            reasons.Add("craft_storage_item_recipe_not_ready");
        }
        ...
        if (!string.Equals(
            ...
            !string.Equals(
                ...
                expectedIngredients,
                StringComparison.Ordinal) ||
            !ReadBool(row.Value, "ordinary_material_storage"))
        {
            reasons.Add(
                "craft_storage_item_projection_drifted");
        }

        if (usesWorkbench)
        {
            ValidateStorageWorkbenchBinding(
                action,
                source.Value,
                reasons);
        }
        ...
        if (!demand.AcquisitionRequired ||
            ...
            ReadIntParameter(
                ...
                demand.InventoryOrdinaryStorageCount ||
            ReadIntParameter(
                action,
                "usable_ordinary_storage_count") !=
                demand.ImmediatelyUsableOrdinaryAccessPointCount ||
            ReadIntParameter(
                action,
        ...
        {
            reasons.Add(
                "craft_storage_item_material_reservation_projection_drifted");
        }
    ...
    private static void ValidateStorageWorkbenchBinding(
        SmallModelAction action,
        JsonElement source,
        ICollection<string> reasons)
    {
        ...
        {
            reasons.Add(
                "craft_storage_item_workbench_projection_drifted");
        }
    }

    private static JsonElement? StorageCraftingRow(
        SnapshotEnvelope snapshot,
        string? recipeName)
    {
    ...
    private static JsonElement? StorageWorkbenchCraftingSource(
        JsonElement row,
