...
public sealed partial class PlayerReadAdapter : ReadAdapterBase
{
    ...
    public override StateAdapterResult Collect(long tick)
    {
        ...
        var playerFields = new Dictionary<string, object>
        {
            ...
            ["crab_pot_network"] = Field(ReadCrabPotNetwork(player), "all persistent GameLocation.objects exact base CrabPot rows; owner professions; bait/ready/output lifecycle; tile-native fish area and Data/Fish trap production domain", tick),
            ...
            ["tailoring"] = Field(ReadTailoringContext(player), "live Data/TailoringRecipes and context tags; inventory input pairs; unlocked Action=Tailoring and placed (BC)247 endpoints; exact boots/recipe output or complete native random result domain; dye branches excluded", tick),
