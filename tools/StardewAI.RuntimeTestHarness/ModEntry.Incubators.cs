...
{
    private static bool IsNativeIncubatorBirthMessageOpen()
    {
        return Game1.currentLocation is AnimalHouse house &&
            ...
            house.objects.Values.Any(machine =>
            ...
            TrainingExecutionRequest request)
    {
        ...
        var machineId = string.IsNullOrWhiteSpace(
                request.ExpectedShopId)
            ? "(BC)101"
            : request.ExpectedShopId.StartsWith(
                "(",
                StringComparison.Ordinal)
                ? request.ExpectedShopId
                : "(BC)" + request.ExpectedShopId;
        var incubator = ItemRegistry.Create<
            StardewValley.Object>(machineId);
        if (incubator.GetMachineData()?.IsIncubator != true)
        {
            return BlockedWithPrimitive(
                request,
                "debug_setup_incubator_hatch_naming",
                "incubator.ready=true;native_animalNaming_event=true",
                IncubatorHatchObservedEffect(),
                "incubator_hatch_fixture_machine_not_incubator");
        }
        ...
        return new TrainingExecutionResult
        {
            ...
            PrimitiveVerificationReasons = verified
                ? new[]
                {
                    ...
                    "machine_qualified_item_id=" + machineId,
